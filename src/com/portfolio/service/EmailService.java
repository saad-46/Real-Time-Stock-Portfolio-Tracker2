...
public class EmailService {
    ...
    private String senderPassword = "YOUR_GMAIL_APP_PASSWORD_HERE";  // ⚠️ REPLACE WITH APP PASSWORD!
    ...
    public void sendDailyPortfolioUpdate(String recipientEmail, PortfolioService portfolioService) {
        try {
            ...
            // Generate HTML
            String emailBody = generateEmailHTML(recipientEmail, totalValue, totalInvestment, profitLoss, profitPercent, items);
    ...
    private String generateEmailHTML(String recipientEmail, double totalValue, double totalInvestment, 
        ...
        html.append("<div style='max-width: 600px; margin: 0 auto; background: white; border-radius: 15px; padding: 30px;'>");
        ...
        html.append("<td style='padding: 10px;'><div style='background: #667eea; padding: 20px; border-radius: 12px; color: white; text-align: center;'>");
        html.append("<div style='font-size: 14px;'>TOTAL VALUE</div>");
        html.append("<div style='font-size: 28px; font-weight: bold;'>₹").append(String.format("%.2f", totalValue)).append("</div></div></td>");
        
        html.append("<td style='padding: 10px;'><div style='background: #4facfe; padding: 20px; border-radius: 12px; color: white; text-align: center;'>");
        html.append("<div style='font-size: 14px;'>INVESTED</div>");
        html.append("<div style='font-size: 28px; font-weight: bold;'>₹").append(String.format("%.2f", totalInvestment)).append("</div></div></td>");
        
        html.append("</tr><tr>");
        
        String plColor = profitLoss >= 0 ? "#4ade80" : "#f87171";
        html.append("<td style='padding: 10px;'><div style='background: ").append(plColor).append("; padding: 20px; border-radius: 12px; color: white; text-align: center;'>");
        html.append("<div style='font-size: 14px;'>PROFIT/LOSS</div>");
        html.append("<div style='font-size: 28px; font-weight: bold;'>₹").append(String.format("%.2f", profitLoss)).append("</div></div></td>");
        
        html.append("<td style='padding: 10px;'><div style='background: ").append(plColor).append("; padding: 20px; border-radius: 12px; color: white; text-align: center;'>");
        html.append("<div style='font-size: 14px;'>RETURN</div>");
        html.append("<div style='font-size: 28px; font-weight: bold;'>").append(String.format("%.2f%%", profitPercent)).append("</div></div></td>");
        ...
        if (!items.isEmpty()) {
            ...
            for (PortfolioItem item : items) {
                ...
                html.append("<td style='padding: 12px;'><strong>").append(item.getStock().getSymbol()).append("</strong></td>");
                html.append("<td style='padding: 12px; text-align: center;'>").append(item.getQuantity()).append("</td>");
                html.append("<td style='padding: 12px; text-align: right;'>₹").append(String.format("%.2f", item.getStock().getCurrentPrice())).append("</td>");
        ...
        // Alerts
        html.append("<div style='background: #fbbf24; padding: 20px; margin: 30px 0; border-radius: 12px; color: white;'>");
        ...
        // Footer
        html.append("<div style='text-align: center; margin-top: 30px; padding-top: 20px; border-top: 2px solid #e2e8f0;'>");
